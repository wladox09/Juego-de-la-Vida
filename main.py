...
status = numpy.zeros((nX,nY)) # Intialize status of cells
...
while running:
    ...
    newStatus = numpy.copy(status) # Copy status
    ...
    for event in pygame.event.get():
        ...
        if sum(mouseClick) > 0:
            ...
            x, y = int(numpy.floor(posX/xSize)), int(numpy.floor(posY/ySize))
            #newStatus[x,y] = numpy.abs(newStatus[x,y]-1)
